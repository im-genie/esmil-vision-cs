...
async def create_entry(payload: dict, u: str = Depends(current_user)):
    ...
    proc_label = "Roll Press" if entry.get("proc") == "RollPress" else entry.get("proc", "")
    ...
    lot        = entry.get("lot", "")
    title = f"\U0001F514 {proc_label} · {ca_label} · L{line}"
    body  = f"{vision} · {action}\n\uC791\uC131\uC790: {reviewer}"
    if lot:
        body += f" · Lot {lot}"
...
async def send_push_to_roles(title: str, body: str, data: dict = None):
    ...
    ensure_db()
    try:
        # Collect (doc_id, token) for all notify-role devices
        pairs = []
        seen = set()
        ...
        # Prune tokens that are no longer valid (uninstalled / expired)
        try:
...
async def register_fcm_token(payload: dict, u: str = Depends(current_user)):
    """Save FCM token for THIS device (so multiple devices per user all get push)."""
    ensure_db()
    ...
    doc_id = device or token  # one doc per device (fallback: per token)
...
async def unregister_fcm_token(deviceId: str = "", u: str = Depends(current_user)):
    ...
    try:
        ...
        # also clean up any legacy doc keyed by username
